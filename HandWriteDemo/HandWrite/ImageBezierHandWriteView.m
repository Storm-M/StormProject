...
- (void)calucatePath:(NSArray *)touchArray {
    ...
    for (int i = 1; i < touchArray.count; i++) {
        ...
        CGPoint prevPoint = CGPointFromString(touchArray[i-1]);
        CGPoint firstPoint = prevPoint;
        if (i > 1) {
            firstPoint = CGPointFromString(touchArray[i-2]);
        }
        CGPoint currentPoint = CGPointFromString(touchArray[i]);
        
        CGPoint mid1 = midPoint(prevPoint, firstPoint);
        CGPoint mid2 = midPoint(currentPoint, prevPoint);
        [path1 moveToPoint:mid1];
        [path1 addQuadCurveToPoint:mid2 controlPoint:prevPoint];
        [path appendPath:path1];
    }
    
    
    
    self.shapeLayer.path          = path.CGPath;                    // 从贝塞尔曲线获取到形状
    //  [self drawView:array];
}
